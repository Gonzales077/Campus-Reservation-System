...
<div class="full-screen-wrapper">
    <div class="admin-top-bar">
        <h2><i class="fas fa-edit"></i> Edit Facility: <span style="color: #64748b;">{{ $facility->name }}</span></h2>
        <div>
                ...
                <div class="row g-4">
                    <div class="col-lg-7">
                        <div class="mb-4">
                            ...
                            <select class="form-select @error('name') is-invalid @enderror" id="name" name="name" required>
                                @foreach($facilityNames as $facilityName)
                                    <option value="{{ $facilityName }}" {{ old('name', $facility->name) === $facilityName ? 'selected' : '' }}>{{ $facilityName }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" class="form-control" id="location" name="location" value="{{ old('location', $facility->location) }}" required>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <input type="number" class="form-control" id="capacity" name="capacity" value="{{ old('capacity', $facility->capacity) }}" required>
                            </div>
                            <div class="col-md-6 mb-4">
                                <label for="available_hours" class="form-label">Available Hours</label>
                                <input type="number" class="form-control" id="available_hours" name="available_hours" value="{{ old('available_hours', $facility->available_hours) }}" required>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select class="form-select" id="status" name="status" required>
                                <option value="active" {{ old('status', $facility->status) === 'active' ? 'selected' : '' }}>Active / Operational</option>
                                <option value="inactive" {{ old('status', $facility->status) === 'inactive' ? 'selected' : '' }}>Inactive / Maintenance</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="p-4 rounded-4" style="background: #f8fafc; border: 1px solid #e2e8f0;">
                            ...
                            @if($facility->thumbnail)
                                <div class="current-asset-card mb-4">
                                    ...
                                    <img src="{{ asset($facility->thumbnail) }}" alt="Thumbnail">
                                </div>
                            @endif
                                    ...
                                    @foreach($facility->images as $img)
                                        <div class="col-4">
                                            <div class="current-asset-card p-1">
                                                <img src="{{ asset($img) }}" style="height: 60px;">
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
