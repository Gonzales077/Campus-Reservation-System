...
{{-- =====================================================================
     FULL-WIDTH ESCAPE: removes Bootstrap container padding on this page
     ===================================================================== --}}
<style>
...
<div class="admin-dashboard-container">
        ...
        <div class="header-gradient-overlay">
            <div class="header-content-wrapper">
                ...
                <div class="header-right">
                    ...
                    <a href="{{ route('admin.messages.index') }}" class="header-action-btn" style="background:#fff; color:#333;">
                        ...
                        <div class="btn-content">
                            ...
                            @if(isset($unreadMessagesCount) && $unreadMessagesCount > 0)
                                <span style="display:inline-block;background:#e53935;color:#fff;padding:2px 6px;border-radius:12px;margin-left:6px;font-size:12px;">{{ $unreadMessagesCount }}</span>
                            @endif
                        </div>
                    </a>
                </div>
    ...
    @if(session('success'))
        <div class="notification-card notification-success">
            ...
            <div class="notification-content"><p class="notification-message">{{ session('success') }}</p></div>
    ...
    @if(session('error'))
        <div class="notification-card notification-error">
            ...
            <div class="notification-content"><p class="notification-message">{{ session('error') }}</p></div>
            <button class="notification-close"><i class="fas fa-times"></i></button>
        </div>
    @endif
        ...
        <div class="stats-grid-modern">
            <div class="stat-card-modern stat-warning">
                ...
                <div class="stat-content">
                    ...
                    <div class="stat-progress"><div class="progress-bar"><div class="progress-fill" style="width:{{ $pendingReservations->count() > 0 ? '80%' : '0%' }}"></div></div></div>
                </div>
            ...
            <div class="stat-card-modern stat-success">
                ...
                <div class="stat-content">
                    ...
                    <div class="stat-progress"><div class="progress-bar"><div class="progress-fill" style="width:{{ $approvedReservations->count() > 0 ? '60%' : '0%' }}"></div></div></div>
                </div>
            ...
            <div class="stat-card-modern stat-primary">
                ...
                <div class="stat-content">
                    ...
                    <div class="stat-progress"><div class="progress-bar"><div class="progress-fill" style="width:{{ $facilities->where('status','active')->count() / max($facilities->count(),1) * 100 }}%"></div></div></div>
                </div>
            ...
            <div class="stat-card-modern stat-secondary">
                ...
                <div class="stat-content">
                    ...
                    <div class="stat-progress"><div class="progress-bar"><div class="progress-fill" style="width:{{ $allReservations->count() > 0 ? '40%' : '0%' }}"></div></div></div>
                </div>
    ...
    <div class="modern-section-card urgent-section" id="pending-section">
        <div class="section-header-modern">
            ...
            <div class="section-actions-modern">
                <span class="status-count-badge badge-warning"><i class="fas fa-clock"></i> {{ $pendingReservations->count() }} Pending</span>
        ...
        <div class="modern-table-container">
            <table class="modern-admin-table">
                ...
                <tbody id="pendingTbody">
                    ...
                    <tr data-search="{{ strtolower($reservation->facility->name.' '.$reservation->guest_name.' '.$reservation->guest_contact.' '.$reservation->description) }}">
                        <td>
                            <div class="facility-info">
                                <div class="facility-name">{{ $reservation->facility->name }}</div>
                                <div class="facility-meta"><span class="facility-meta-item"><i class="fas fa-users"></i> {{ $reservation->facility->capacity }}</span></div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td><div class="contact-info"><a href="tel:{{ $reservation->guest_contact }}" class="contact-link"><i class="fas fa-phone"></i> {{ $reservation->guest_contact }}</a></div></td>
                        <td><div class="description-content">{{ Str::limit($reservation->description, 60) }}</div></td>
                        <td>
...
@endforeach
    ...
    <div class="form-control-custom bg-light p-2 mb-2" style="border: 1px solid #ddd; border-radius: 5px;">
        <span class="fw-bold">
            {{-- Check if data exists, then format it --}}
            @if($reservation->requested_date)
                {{ \Carbon\Carbon::parse($reservation->requested_date)->format('F j, Y') }}
            @else
    ...
    {{-- This hidden field sends the date back to the 'approveReservation' method --}}
    <input type="hidden" name="available_date" value="{{ $reservation->requested_date }}">
    ...
    <div class="modern-section-card" id="approved-section">
        <div class="section-header-modern">
            ...
            <div class="section-actions-modern">
                <span class="status-count-badge badge-success"><i class="fas fa-check-circle"></i> {{ $approvedReservations->count() }} Active</span>
            </div>
        </div>
        ...
        <div class="modern-table-container">
            <table class="modern-admin-table">
                ...
                <tbody id="approvedTbody">
                    ...
                    <tr data-search="{{ strtolower($reservation->facility->name.' '.$reservation->guest_name.' '.$reservation->guest_contact.' '.$reservation->description.' '.$reservation->facility->location) }}">
                        <td>
                            <div class="facility-info">
                                <div class="facility-name">{{ $reservation->facility->name }}</div>
                                <div class="facility-meta"><span class="facility-meta-item"><i class="fas fa-map-marker-alt"></i> {{ $reservation->facility->location }}</span></div>
                            </div>
                        </td>
                        <td>
                            <div class="guest-info">
                        ...
                        <td><div class="contact-info"><a href="tel:{{ $reservation->guest_contact }}" class="contact-link"><i class="fas fa-phone"></i> {{ $reservation->guest_contact }}</a></div></td>
                        <td><div class="description-content">{{ Str::limit($reservation->description, 50) }}</div></td>
                        <td>
                            <div class="date-display-modern">
                                <div class="date-icon"><i class="fas fa-calendar-day"></i></div>
                                <div class="date-info">
                                    <div class="date-value">{{ $reservation->available_date ? $reservation->available_date->format('M d, Y') : 'Not set' }}</div>
                                    @if($reservation->available_date)
                                    <div class="date-remaining">{{ \Carbon\Carbon::parse($reservation->available_date)->diffForHumans() }}</div>
                                    @endif
                                </div>
                            </div>
                        </td>
    ...
    <div class="modern-section-card" id="google-calendar">
        <div class="section-header-modern">
            ...
            <div class="section-actions-modern">
                <span class="status-count-badge"><i class="fas fa-envelope"></i> {{ auth()->user()->gmail_address ?? 'Not Set' }}</span>
            </div>
        </div>
        @if(!auth()->user()->gmail_address)
        ...
        @else
            <div style="padding:20px;background:#f8f9fa;border-radius:8px;">
                <div style="background:#fff;padding:20px;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <p style="margin:10px 0;font-size:14px;color:#666;"><strong>Your Gmail:</strong> {{ auth()->user()->gmail_address }}</p>
                    ...
                    <div style="background:#f0f4f8;padding:12px;border-radius:4px;border-left:4px solid #1a3a52;">
                        ...
                        <p style="margin:8px 0 0;font-size:13px;color:#555;"><span id="approved-count">{{ $approvedReservations->count() ?? 0 }}</span> facilities currently reserved</p>
                    </div>
                </div>
            </div>
        @endif
    ...
    <div class="modern-section-card" id="all-reservations">
        <div class="section-header-modern">
            ...
            <div class="section-actions-modern">
                <span class="status-count-badge"><i class="fas fa-list"></i> {{ $allReservations->count() }} Total</span>
            </div>
        </div>
            ...
            <table class="modern-admin-table">
                ...
                <tbody id="allTbody">
                    ...
                    <tr class="status-{{ $reservation->status }}"
                        data-search="{{ strtolower($reservation->facility->name.' '.$reservation->guest_name.' '.$reservation->guest_contact.' '.$reservation->description) }}"
                        data-status="{{ $reservation->status }}">
                        <td><div class="facility-info"><div class="facility-name">{{ $reservation->facility->name }}</div></div></td>
                        <td><div class="guest-info"><div class="guest-name">{{ $reservation->guest_name }}</div></div></td>
                        <td><div class="contact-info"><a href="tel:{{ $reservation->guest_contact }}" class="contact-link"><i class="fas fa-phone"></i> {{ $reservation->guest_contact }}</a></div></td>
                        <td><div class="description-content">{{ Str::limit($reservation->description, 50) }}</div></td>
                        <td>
                        ...
                        <td>
                            <div class="date-display-modern">
                                ...
                                <div class="date-info">
                                    <div class="date-value">{{ $reservation->created_at->format('M d, Y') }}</div>
                                    <div class="date-ago">{{ $reservation->created_at->diffForHumans() }}</div>
                                </div>
                            </div>
                        </td>
                    </tr>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="updateGmailForm" action="{{ route('admin.update-gmail') }}" method="POST">
                    @csrf
                    <div class="form-group" style="margin-bottom:15px;">
                        <label for="gmail_address" style="font-weight:600;color:#333;margin-bottom:8px;display:block;">Gmail Address</label>
                        <input type="email" id="gmail_address" name="gmail_address" class="form-control" placeholder="your.email@gmail.com" value="{{ auth()->user()->gmail_address ?? '' }}" required>
